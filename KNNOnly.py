...
# Initialize the KNN background subtractor with custom parameters
knn_bg_subtractor = cv2.createBackgroundSubtractorKNN(detectShadows=False)
...
output_frame_folder = '//home/raspberry/Desktop/sesan project-1/MOG002/data preprocess/KNN/postcard_frames001'
os.makedirs(output_frame_folder, exist_ok=True)

# Create video writer for final combined video
combined_video_path = '/home/raspberry/Desktop/sesan project-1/MOG002/data preprocess/KNN/postcard_video001.mp4'
...
original_video_path = '/home/raspberry/Desktop/sesan project-1/MOG002/data preprocess/KNN/postcard_original_video001.mp4'
...
while time.time() - start_time < capture_duration:
    ...
    # Apply the contour mask to the KNN result
    knn_mask_filtered = cv2.bitwise_and(knn_mask, contour_mask)

    # Save consecutive frames
    output_frame_path = os.path.join(output_frame_folder, f'output_frame_{frame_counter}.png')
    cv2.imwrite(output_frame_path, knn_mask_filtered)

    # Write frames to the final combined video
    combined_video_writer.write(cv2.cvtColor(knn_mask_filtered, cv2.COLOR_GRAY2BGR))
    ...
    predicted_labels.append(np.sum(knn_mask_filtered > 0))

    # Display the combined video in real-time
    cv2.imshow('Combined Video', knn_mask_filtered)
